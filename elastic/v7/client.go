...
var (
	...
	// are routed to it.
	SetSnifferCallback = elastic.SetSnifferCallback
...
func NewClient(shards uint16, options ...elastic.ClientOptionFunc) (*Client, error) {
	client, err := elastic.NewClient(options...)
	if err != nil {
		return nil, err
	}
	if shards == 0 {
		return nil, errors.New("shards cannot be 0")
	}
	return &Client{
		client: client,
		shards: shards,
...
func (c *Client) IndexExists(index string) (bool, error) {
	return c.client.IndexExists(index).Do(context.Background())
}
...
func (c *Client) CreateIndex(index string, mapping string) error {
	...
	body := fmt.Sprintf("{\"mappings\":%s,\"settings\":{\"number_of_replicas\":0,\"number_of_shards\":%d}}", mapping, c.shards)
...
// typ is ignored in v7.
func (c *Client) PutMapping(index string, typ string, mapping string) error {
	res, err := c.client.PutMapping().Index(index).BodyString(mapping).Do(context.Background())
	if err != nil {
...
func (c *Client) GetIndexSummary(index string) (es.IndexSummary, error) {
	...
	stats, err := c.client.IndexStats(index).Do(context.Background())
	if err != nil {
